...
- `MacAutoLockWatch`: Apple Watch app scaffold. Watch quick-lock relay is deferred until the Mac+iPhone BLE path is stable.
...
2. Open the menu bar panel and copy the 4-digit Bluetooth pairing code.
...
The Mac scans for the trusted iPhone Bluetooth advertisement and evaluates the smoothed RSSI against the configured threshold. When the trusted iPhone drops below the threshold, the Mac locks immediately. If the iPhone stops reporting entirely, the Mac falls back to the offline grace timeout.
